...
public class PyralTreeLargeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState OAK_LOG = AlterniaBlocks.pyralLog.getDefaultState();
   private static final BlockState OAK_LEAVES = AlterniaBlocks.pyralLeaves.getDefaultState();

   public PyralTreeLargeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn) {
   ...
   private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<PyralTreeLargeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
      for(PyralTreeLargeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) {
   ...
   private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<PyralTreeLargeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
      for(PyralTreeLargeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_) {
   ...
   public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox p_208519_5_) {
      ...
      if (i == -1) {
         ...
         int i1 = i - 5;
         List<PyralTreeLargeFeature.FoliageCoordinates> list = Lists.newArrayList();
         list.add(new PyralTreeLargeFeature.FoliageCoordinates(position.up(i1), l));
               ...
               for(int j1 = 0; j1 < k; ++j1) {
                  ...
                  if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, p_208519_5_) == -1) {
                     ...
                     if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, p_208519_5_) == -1) {
                        list.add(new PyralTreeLargeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
                     }
                  }
               }
            }
